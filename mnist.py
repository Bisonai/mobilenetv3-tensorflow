...
def download_mnist(data_path: Path):
    ...
    def save_data(data, labels, split_name):
        ...
        for label_id in tqdm(range(10)):
            ...
            label_path.mkdir()
            for img_id, img in enumerate(data[labels == label_id]):
...
def _parse_mnist(filename, label, shape):
    height, width = shape
    image_string = tf.read_file(filename)
    img = tf.image.decode_png(image_string, channels=1)
...
def mnist(
    ...
    train_data, num_train_data = build_mnist(
        batch_size=train_batch_size,
        shape=(height, width),
        training=True,
    )

    test_data, num_test_data = build_mnist(
